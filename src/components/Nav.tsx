		...
		<nav className="mx-auto max-w-5xl flex flex-wrap gap-3 sm:gap-4 p-4 items-center">
			...
			{isAuthenticated ? (
				<>
					...
					{isAdmin && (
						<>
							<Link href="/admin" className="text-sm hover:text-orange-600">
								Admin
							</Link>
							<Link href="/admin/settings" className="text-sm hover:text-orange-600 hidden sm:inline">
								Settings
							</Link>
						</>
					)}
					<button onClick={handleLogout} className="text-sm ml-auto hover:text-orange-600">
						Logout
					</button>
				</>
			) : (
				<>
					<Link href="/auth/login" className="text-sm ml-auto hover:text-orange-600">
						Login
					</Link>
					<Link href="/auth/register" className="text-sm hover:text-orange-600">
						Register
					</Link>
				</>
			)}
